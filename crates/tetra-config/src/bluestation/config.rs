        ...
        match self.phy_io.backend {
            PhyBackend::SoapySdr => {
                ...
                let config_count = [
                    ...
                    soapy_cfg.io_cfg.iocfg_plutosdr.is_some(),
                ]
                .iter()
                .filter(|&&x| x)
                .count();
                if config_count != 1 {
                    return Err(
                        "soapysdr backend requires exactly one hardware configuration (iocfg_usrpb2xx, iocfg_limesdr, iocfg_sxceiver, or iocfg_plutosdr)",
                    );
                }
            }
