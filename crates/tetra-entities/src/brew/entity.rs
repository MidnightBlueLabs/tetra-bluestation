...
// Scanning for SDS staging expiry every TDMA tick is overkill and can add jitter.
// Keep it throttled so LST transitions stay stable on slower hosts.
const SDS_EXPIRE_SCAN_INTERVAL: Duration = Duration::from_millis(250);
...
pub struct BrewEntity {
    ...
    // Throttle SDS staging expiry scans
    last_sds_expire_scan: Instant,
...
impl BrewEntity {
    pub fn new(config: SharedConfig) -> Self {
        ...
        Self {
            ...
            last_sds_expire_scan: Instant::now(),
            connected: false,
        ...
        while let Ok(event) = self.event_receiver.try_recv() {
            match event {
                ...
                BrewEvent::Disconnected(reason) => {
                    ...
                    // LST/fallback stability: drop any in-flight SDS staging when backhaul disappears.
                    // Keeping these around causes unnecessary per-tick work and can create bursts of
                    // error logging / failed reports during reconnect storms.
                    self.sds_sessions.clear();
                    self.sds_pending.clear();
                    self.pending_sds_pid04_reports.clear();
                    self.next_sds_dl_ts1 = None;
                    self.last_sds_expire_scan = Instant::now();
                }
...
impl TetraEntityTrait for BrewEntity {
    ...
    fn tick_start(&mut self, queue: &mut MessageQueue, ts: TdmaTime) {
        ...
        // Throttle SDS staging expiry scans: doing this every TDMA tick can add jitter.
        // Only scan when connected and there is anything pending.
        if self.connected
            && (!self.sds_sessions.is_empty() || !self.sds_pending.is_empty() || !self.pending_sds_pid04_reports.is_empty())
        {
            let now = Instant::now();
            if now.duration_since(self.last_sds_expire_scan) >= SDS_EXPIRE_SCAN_INTERVAL {
                self.last_sds_expire_scan = now;
                self.expire_sds_staging();
            }
        } else {
            // Keep timestamp fresh to avoid large catch-up scans.
            self.last_sds_expire_scan = Instant::now();
        }
