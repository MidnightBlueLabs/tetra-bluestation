...
impl BsFragger {
    ...
    fn get_resource_chunk(&mut self, mac_block: &mut BitBuffer) -> bool {
        ...
        let min_len_bits = hdr_len_bits + sdu_len_bits;
        let total_len_bits = min_len_bits + num_fill_bits;
        ...
        if total_len_bits <= slot_cap_bits {
            ...
            true
        } else if min_len_bits <= slot_cap_bits {
            // SDU fits, but full fill bits do not. Write SDU and best-effort fill bits
            // (at least the leading '1') so the receiver can strip padding.
            self.resource.length_ind = total_len_bytes as u8;
            self.resource.fill_bits = num_fill_bits > 0;
            ...
            self.resource.to_bitbuf(mac_block);
            mac_block.copy_bits(&mut self.sdu, sdu_len_bits);
            // Fill remaining space with a fill-bit pattern as far as it fits.
